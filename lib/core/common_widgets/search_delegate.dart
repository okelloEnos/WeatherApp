...
Future<T?> showCustomSearch<T>({
  ...
  delegate._currentBody = _SearchCustomBody.suggestions;
  return Navigator.of(context, rootNavigator: useRootNavigator).push(_SearchCustomPageRoute<T>(
    delegate: delegate,
  ));
}
...
abstract class SearchCustomDelegate<T> {
  ...
  ThemeData appBarTheme(BuildContext context) {
    ...
    return theme.copyWith(
      appBarTheme: AppBarTheme(
        ...
        backgroundColor: colorScheme.brightness == Brightness.dark ? Colors.grey[900] : Theme.of(context).colorScheme.primary,
  ...
  void showResults(BuildContext context) {
    _focusNode?.unfocus();
    _currentBody = _SearchCustomBody.results;
  }
  ...
  void showSuggestions(BuildContext context) {
    ...
    _currentBody = _SearchCustomBody.suggestions;
  }
  ...
  final ValueNotifier<_SearchCustomBody?> _currentBodyNotifier = ValueNotifier<_SearchCustomBody?>(null);

  _SearchCustomBody? get _currentBody => _currentBodyNotifier.value;
  set _currentBody(_SearchCustomBody? value) {
    _currentBodyNotifier.value = value;
  }

  _SearchCustomPageRoute<T>? _route;
...
class _SearchCustomPageRoute<T> extends PageRoute<T> {
  _SearchCustomPageRoute({
...
class _SearchCustomPage<T> extends StatefulWidget {
  const _SearchCustomPage({
    required this.delegate,
  ...
  @override
  State<StatefulWidget> createState() => _SearchCustomPageState<T>();
}

class _SearchCustomPageState<T> extends State<_SearchCustomPage<T>> {
  ...
  void _onAnimationStatusChanged(AnimationStatus status) {
    ...
    widget.animation.removeStatusListener(_onAnimationStatusChanged);
    if (widget.delegate._currentBody == _SearchCustomBody.suggestions) {
      focusNode.requestFocus();
    }
  }

  @override
  void didUpdateWidget(_SearchCustomPage<T> oldWidget) {
  ...
  void _onFocusChanged() {
    if (focusNode.hasFocus && widget.delegate._currentBody != _SearchCustomBody.suggestions) {
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.delegate._currentBody) {
      case _SearchCustomBody.suggestions:
        body = KeyedSubtree(
          key: const ValueKey<_SearchCustomBody>(_SearchCustomBody.suggestions),
          child: widget.delegate.buildSuggestions(context),
        );
      case _SearchCustomBody.results:
        body = KeyedSubtree(
          key: const ValueKey<_SearchCustomBody>(_SearchCustomBody.results),
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            title: TextField(
              ...
              prefixIcon: Icon(Icons.search, color: Colors.black.withOpacity(0.2), size: 16.0),
                suffixIcon: IconButton(
                  icon: Icon(Icons.cancel_outlined, color: Colors.black.withOpacity(0.6), size: 20.0),
                  onPressed: () {
